...
class PXEBoot(pxe.PXEBoot):
    ...
    def _plug_provisioning(self, task, **kwargs):
        ...
        common.add_vnic(task, name, port['port']['mac_address'], seg_id, True)

        new_port = objects.Port(
            task.context, node_id=task.node.id,
            address=port['port']['mac_address'],
            extra={"vif_port_id": port['port']['id'], "deploy": True})
        new_port.create()
    ...
    def prepare_ramdisk(self, task, ramdisk_params):
        ...
        pxe_options = pxe._build_pxe_config_options(node, pxe_info,
                                                    task.context)
