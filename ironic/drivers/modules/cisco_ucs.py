...
REQUIRED_PROPERTIES = {
    'ucsm_address': _("IP or hostname of the UCS Manager Required."),
    ...
    try:
        ...
        if processor_units.errorCode == 0:
            for p_unit in processor_units.OutConfigs.GetChild():
                ...
                units['AdaptorUnit-%s' % (p_unit.getattr(ProcessorUnit.ID))] =
                    unit
    ...
    try:
        ...
        if (arrays.errorCode == 0):
            for array in arrays.OutConfigs.GetChild():
                ...
                arrays['MemoryArray-%s' % (array.getattr(MemoryArray.ID))] =
                    unit
        ...
        if (local_disks.errorCode == 0):
            for l_disk in local_disks.OutConfigs.GetChild():
                ...
                disk['connectionProtocol'] =
                    l_disk.getattr(StorageLocalDisk.CONNECTION_PROTOCOL)
                disk['model'] = l_disk.getattr(StorageLocalDisk.MODEL)
                disk['numberOfBlocks'] =
                    l_disk.getattr(StorageLocalDisk.NUMBER_OF_BLOCKS)
                ...
                disks['StorageLocalDisk-%s' % \
                    (l_disk.getattr(StorageLocalDisk.ID))] = disk
    except UcsException as ex:
        raise exception.IronicException("Cisco Driver: (%s)" %ex)
    return disks
    ...
    try:
        ...
        if (adaptor_units.errorCode == 0):
            for a_unit in adaptor_units.OutConfigs.GetChild():
                ...
                units['AdaptorUnit-%s' % (a_unit.getattr(AdaptorUnit.ID))] =
                    unit
    except UcsException as ex:
        raise exception.IronicException("Cisco Driver: (%s)" %ex)
...
class CiscoIronicDriverHelper(object):
        ...
        try:
            ...
            if disks.errorCode == 0:
                for local_disk in disks.OutConfigs.GetChild():
                    if local_disk.getattr(StorageLocalDisk.SIZE) != \
                           StorageLocalDisk.CONST_BLOCK_SIZE_UNKNOWN:
                        local += int(local_disk.getattr(StorageLocalDisk.SIZE))
                    LOG.error('Disk:%s size:%s' % \
                               (local_disk.getattr(StorageLocalDisk.DN),
                               local_disk.getattr(StorageLocalDisk.SIZE))
                             )
        except UcsException as ex:
            raise exception.IronicException("Cisco driver: %s" % ex)
        if local_gb != 0:
    ...
    def update_ironic_db(self, mac, ls_server, compute_blade):
        ...
        try:
            ...
            if not power:
                ...
                LOG.error("State:%s" % (ls_power[0].getattr(LsPower.STATE)))
                if power[0].getattr(LsPower.STATE) == None:
                    power_state = states.ERROR
                if power[0].getattr(LsPower.STATE) == LsPower.CONST_STATE_DOWN:
                    power_state = states.POWER_OFF
                elif power[0].getattr(LsPower.STATE) == LsPower.CONST_STATE_UP:
        ...
        node = {'driver': 'pxe_cisco',
                'driver_info': 
                    {'service_profile': ls_server.getattr(LsServer.DN),
                ...
                'properties': 
                    {'memory_mb': 
                         compute_blade.getattr(ComputeBlade.TOTAL_MEMORY),
                ...
                if ((lsboot_lan != 0)
                    ...
                    for image_path in lsboot_lan.GetChild():
                        if ((image_path != 0)):
                            ...
                            if (vnic != 0):
                                LOG.debug("MAC" +
                                    vnic[0].getattr(VnicEther.ADDR))
                                mac.insert(int(\
                                    vnic[0].getattr(VnicEther.OPER_ORDER))-1,
        ...
        try:
            ...
            if not ls_power:
                ...
                if ls_power[0].getattr(LsPower.STATE) == None:
                    power_status = states.ERROR
                if ls_power[0].getattr(LsPower.STATE) == \
                   LsPower.CONST_STATE_DOWN:
                    power_status = states.POWER_OFF
                elif ls_power[0].getattr(LsPower.STATE) == \
                    LsPower.CONST_STATE_UP:
    ...
    def _reboot(self, task, timeout=None):
        ...
        if p_state == states.POWER_OFF:
            ...
            self._set_power_state(task, 
                                  LsPower.CONST_STATE_HARD_RESET_IMMEDIATE)
        ...
        if ls_server and len(ls_server) == 1:
            ...
            if fault_insts:
                for fault_inst in fault_insts.OutConfigs.GetChild():
                    fault_details = {
                        FaultInst.CHANGE_SET: \
                            fault_inst.getattr(FaultInst.CHANGE_SET),
                        FaultInst.DESCR : fault_inst.getattr(FaultInst.DESCR),
                        FaultInst.LAST_TRANSITION: \
                            fault_inst.getattr(FaultInst.LAST_TRANSITION),
                        FaultInst.RN: fault_inst.getattr(FaultInst.RN),
                        FaultInst.TYPE: fault_inst.getattr(FaultInst.TYPE),
                        FaultInst.SEVERITY: \
                            fault_inst.getattr(FaultInst.SEVERITY),
                        ...
                        FaultInst.CREATED: \
                            fault_inst.getattr(FaultInst.CREATED),
                        FaultInst.ACK: fault_inst.getattr(FaultInst.ACK),
                        FaultInst.RULE: fault_inst.getattr(FaultInst.RULE),
                        FaultInst.ORIG_SEVERITY: \
                            fault_inst.getattr(FaultInst.ORIG_SEVERITY),
                        FaultInst.PREV_SEVERITY: \
                            fault_inst.getattr(FaultInst.PREV_SEVERITY),
                        FaultInst.CODE: fault_inst.getattr(FaultInst.CODE),
                        FaultInst.HIGHEST_SEVERITY: \
                            fault_inst.getattr(FaultInst.HIGHEST_SEVERITY),
                        FaultInst.ID: fault_inst.getattr(FaultInst.ID),
                        FaultInst.OCCUR: fault_inst.getattr(FaultInst.OCCUR)
                        }
                    params['faults'][fault_inst.getattr(FaultInst.DN)]  = \
                        fault_details
        return params
    ...
    def get_temperature_stats(self, task):
        ...
        if server and len(server) == 1:
            if 'blade' in server[0].getattr(LsServer.PN_DN):
                #make ComputeMbTempStats Mo dn.
                dn = server[0].getattr(LsServer.PN_DN) + '/board/temp-stats'
                t_stats = handle.GetManagedObject(
                                None,
                                ComputeMbTempStats.ClassId(),
                                {ComputeMbTempStats.DN: dn},
                                inHierarchical=YesOrNo.FALSE,
                                dumpXml=YesOrNo.TRUE
                                #dumpXml=CONF.cisco.dump_xml
                                )
                if t_stats and len(t_stats) == 1 :
                    temp_stats = {
                             ComputeMbTempStats.DN: \
                                 t_stats[0].getattr(ComputeMbTempStats.DN),
                             ComputeMbTempStats.FM_TEMP_SEN_IO: \
                   mb_temp_stats[0].getattr(ComputeMbTempStats.FM_TEMP_SEN_IO),
                             ComputeMbTempStats.FM_TEMP_SEN_IO_AVG: \
               mb_temp_stats[0].getattr(ComputeMbTempStats.FM_TEMP_SEN_IO_AVG),
                             ComputeMbTempStats.FM_TEMP_SEN_IO_MAX: \
               mb_temp_stats[0].getattr(ComputeMbTempStats.FM_TEMP_SEN_IO_MAX),
                             ComputeMbTempStats.FM_TEMP_SEN_IO_MIN: \
               mb_temp_stats[0].getattr(ComputeMbTempStats.FM_TEMP_SEN_IO_MIN),
                             ComputeMbTempStats.FM_TEMP_SEN_REAR: \
               mb_temp_stats[0].getattr(ComputeMbTempStats.FM_TEMP_SEN_REAR),
                             ComputeMbTempStats.FM_TEMP_SEN_REAR_AVG: \
             mb_temp_stats[0].getattr(ComputeMbTempStats.FM_TEMP_SEN_REAR_AVG),
                             ComputeMbTempStats.FM_TEMP_SEN_REAR_L: \
               mb_temp_stats[0].getattr(ComputeMbTempStats.FM_TEMP_SEN_REAR_L),
                             ComputeMbTempStats.FM_TEMP_SEN_REAR_LAVG: \
            mb_temp_stats[0].getattr(ComputeMbTempStats.FM_TEMP_SEN_REAR_LAVG),
                             ComputeMbTempStats.FM_TEMP_SEN_REAR_LMAX: \
            mb_temp_stats[0].getattr(ComputeMbTempStats.FM_TEMP_SEN_REAR_LMAX),
                             ComputeMbTempStats.FM_TEMP_SEN_REAR_LMIN: \
            mb_temp_stats[0].getattr(ComputeMbTempStats.FM_TEMP_SEN_REAR_LMIN),
                             ComputeMbTempStats.FM_TEMP_SEN_REAR_MAX: \
             mb_temp_stats[0].getattr(ComputeMbTempStats.FM_TEMP_SEN_REAR_MAX),
                             ComputeMbTempStats.FM_TEMP_SEN_REAR_MIN: \
             mb_temp_stats[0].getattr(ComputeMbTempStats.FM_TEMP_SEN_REAR_MIN),
                             ComputeMbTempStats.FM_TEMP_SEN_REAR_R: \
               mb_temp_stats[0].getattr(ComputeMbTempStats.FM_TEMP_SEN_REAR_R),
                             ComputeMbTempStats.FM_TEMP_SEN_REAR_RAVG: \
            mb_temp_stats[0].getattr(ComputeMbTempStats.FM_TEMP_SEN_REAR_RAVG),
                             ComputeMbTempStats.FM_TEMP_SEN_REAR_RMAX: \
            mb_temp_stats[0].getattr(ComputeMbTempStats.FM_TEMP_SEN_REAR_RMAX),
                             ComputeMbTempStats.FM_TEMP_SEN_REAR_RMIN: \
            mb_temp_stats[0].getattr(ComputeMbTempStats.FM_TEMP_SEN_REAR_RMIN),
                             ComputeMbTempStats.SUSPECT: \
                          mb_temp_stats[0].getattr(ComputeMbTempStats.SUSPECT),
                             ComputeMbTempStats.THRESHOLDED: \
                      mb_temp_stats[0].getattr(ComputeMbTempStats.THRESHOLDED),
                             ComputeMbTempStats.TIME_COLLECTED: \
                    mb_temp_stats[0].getattr(ComputeMbTempStats.TIME_COLLECTED)
                             }
                    params['temp_stats'] = temp_stats
            elif 'rack' in ls_server[0].getattr(LsServer.PN_DN):
                dn = ls_server[0].getattr(LsServer.PN_DN) + '/board/temp-stats'
                mb_temp_stats = handle.GetManagedObject(
                                    None,
                                    ComputeRackUnitMbTempStats.ClassId(),
                                    {ComputeRackUnitMbTempStats.DN: dn},
                                    inHierarchical=YesOrNo.FALSE,
                                    dumpXml=YesOrNo.TRUE
                                    )
                if mb_temp_stats and len(mb_temp_stats) == 1 :
                    temp_stats = {
                       ...
                       mb_temp_stats[0].getattr(ComputeRackUnitMbTempStats.DN),
                             ComputeRackUnitMbTempStats.AMBIENT_TEMP: \
             mb_temp_stats[0].getattr(ComputeRackUnitMbTempStats.AMBIENT_TEMP),
                             ComputeRackUnitMbTempStats.AMBIENT_TEMP_AVG: \
         mb_temp_stats[0].getattr(ComputeRackUnitMbTempStats.AMBIENT_TEMP_AVG),
                             ComputeRackUnitMbTempStats.AMBIENT_TEMP_MAX: \
         mb_temp_stats[0].getattr(ComputeRackUnitMbTempStats.AMBIENT_TEMP_MAX),
                             ComputeRackUnitMbTempStats.AMBIENT_TEMP_MIN: \
         mb_temp_stats[0].getattr(ComputeRackUnitMbTempStats.AMBIENT_TEMP_MIN),
                             ComputeRackUnitMbTempStats.FRONT_TEMP: \
               mb_temp_stats[0].getattr(ComputeRackUnitMbTempStats.FRONT_TEMP),
                             ComputeRackUnitMbTempStats.FRONT_TEMP_AVG: \
           mb_temp_stats[0].getattr(ComputeRackUnitMbTempStats.FRONT_TEMP_AVG),
                             ComputeRackUnitMbTempStats.FRONT_TEMP_MAX: \
           mb_temp_stats[0].getattr(ComputeRackUnitMbTempStats.FRONT_TEMP_MAX),
                             ComputeRackUnitMbTempStats.FRONT_TEMP_MIN: \
           mb_temp_stats[0].getattr(ComputeRackUnitMbTempStats.FRONT_TEMP_MIN),
                             ComputeRackUnitMbTempStats.INTERVALS: \
                mb_temp_stats[0].getattr(ComputeRackUnitMbTempStats.INTERVALS),
                             ComputeRackUnitMbTempStats.IOH1_TEMP: \
                mb_temp_stats[0].getattr(ComputeRackUnitMbTempStats.IOH1_TEMP),
                             ComputeRackUnitMbTempStats.IOH1_TEMP_AVG: \
            mb_temp_stats[0].getattr(ComputeRackUnitMbTempStats.IOH1_TEMP_AVG),
                             ComputeRackUnitMbTempStats.IOH1_TEMP_MAX: \
            mb_temp_stats[0].getattr(ComputeRackUnitMbTempStats.IOH1_TEMP_MAX),
                             ComputeRackUnitMbTempStats.IOH1_TEMP_MIN: \
            mb_temp_stats[0].getattr(ComputeRackUnitMbTempStats.IOH1_TEMP_MIN),
                             ComputeRackUnitMbTempStats.IOH2_TEMP: \
                mb_temp_stats[0].getattr(ComputeRackUnitMbTempStats.IOH2_TEMP),
                             ComputeRackUnitMbTempStats.IOH2_TEMP_AVG: \
            mb_temp_stats[0].getattr(ComputeRackUnitMbTempStats.IOH2_TEMP_AVG),
                             ComputeRackUnitMbTempStats.IOH2_TEMP_MAX: \
            mb_temp_stats[0].getattr(ComputeRackUnitMbTempStats.IOH2_TEMP_MAX),
                             ComputeRackUnitMbTempStats.IOH2_TEMP_MIN: \
            mb_temp_stats[0].getattr(ComputeRackUnitMbTempStats.IOH2_TEMP_MIN),
                             ComputeRackUnitMbTempStats.REAR_TEMP: \
                mb_temp_stats[0].getattr(ComputeRackUnitMbTempStats.REAR_TEMP),
                             ComputeRackUnitMbTempStats.REAR_TEMP_AVG: \
            mb_temp_stats[0].getattr(ComputeRackUnitMbTempStats.REAR_TEMP_AVG),
                             ComputeRackUnitMbTempStats.REAR_TEMP_MAX: \
            mb_temp_stats[0].getattr(ComputeRackUnitMbTempStats.REAR_TEMP_MAX),
                             ComputeRackUnitMbTempStats.REAR_TEMP_MIN: \
            mb_temp_stats[0].getattr(ComputeRackUnitMbTempStats.REAR_TEMP_MIN),
                             ComputeRackUnitMbTempStats.SUSPECT: \
                  mb_temp_stats[0].getattr(ComputeRackUnitMbTempStats.SUSPECT),
                             ComputeRackUnitMbTempStats.THRESHOLDED: \
              mb_temp_stats[0].getattr(ComputeRackUnitMbTempStats.THRESHOLDED),
                             ComputeRackUnitMbTempStats.TIME_COLLECTED: \
            mb_temp_stats[0].getattr(ComputeRackUnitMbTempStats.TIME_COLLECTED)
                             }
    ...
    def get_power_stats(self, task):
        ...
        if ls_server and len(ls_server) == 1:
            #make ComputeMbTempStats Mo dn.
            dn = ls_server[0].getattr(LsServer.PN_DN) + '/board/power-stats'
            mb_power_stats = handle.GetManagedObject(
                                 None,
                                 ComputeMbPowerStats.ClassId(),
                                 {ComputeMbPowerStats.DN: dn},
                                 dumpXml=YesOrNo.TRUE
                                 #dumpXml=CONF.cisco.dump_xml
                                 )
            if mb_power_stats and len(mb_power_stats) == 1 :
                power_stats = {
                    str(ComputeMbPowerStats.DN): \
                             mb_power_stats[0].getattr(ComputeMbPowerStats.DN),
                    str(ComputeMbPowerStats.CONSUMED_POWER): \
                 mb_power_stats[0].getattr(ComputeMbPowerStats.CONSUMED_POWER),
                    str(ComputeMbPowerStats.CONSUMED_POWER_AVG): \
             mb_power_stats[0].getattr(ComputeMbPowerStats.CONSUMED_POWER_AVG),
                    str(ComputeMbPowerStats.CONSUMED_POWER_MAX): \
             mb_power_stats[0].getattr(ComputeMbPowerStats.CONSUMED_POWER_MAX),
                    str(ComputeMbPowerStats.CONSUMED_POWER_MIN): \
             mb_power_stats[0].getattr(ComputeMbPowerStats.CONSUMED_POWER_MIN),
                    str(ComputeMbPowerStats.INPUT_CURRENT): \
                  mb_power_stats[0].getattr(ComputeMbPowerStats.INPUT_CURRENT),
                    str(ComputeMbPowerStats.INPUT_CURRENT_AVG): \
              mb_power_stats[0].getattr(ComputeMbPowerStats.INPUT_CURRENT_AVG),
                    str(ComputeMbPowerStats.INPUT_CURRENT_MAX): \
              mb_power_stats[0].getattr(ComputeMbPowerStats.INPUT_CURRENT_MAX),
                    str(ComputeMbPowerStats.INPUT_CURRENT_MIN): \
              mb_power_stats[0].getattr(ComputeMbPowerStats.INPUT_CURRENT_MIN),
                    str(ComputeMbPowerStats.INPUT_VOLTAGE): \
                  mb_power_stats[0].getattr(ComputeMbPowerStats.INPUT_VOLTAGE),
                    str(ComputeMbPowerStats.INPUT_VOLTAGE_AVG): \
              mb_power_stats[0].getattr(ComputeMbPowerStats.INPUT_VOLTAGE_AVG),
                    str(ComputeMbPowerStats.INPUT_VOLTAGE_MAX): \
              mb_power_stats[0].getattr(ComputeMbPowerStats.INPUT_VOLTAGE_MAX),
                    str(ComputeMbPowerStats.INPUT_VOLTAGE_MIN): \
              mb_power_stats[0].getattr(ComputeMbPowerStats.INPUT_VOLTAGE_MIN),
                    str(ComputeMbPowerStats.SUSPECT): \
                        mb_power_stats[0].getattr(ComputeMbPowerStats.SUSPECT),
                    str(ComputeMbPowerStats.THRESHOLDED): \
                    mb_power_stats[0].getattr(ComputeMbPowerStats.THRESHOLDED),
                    str(ComputeMbPowerStats.TIME_COLLECTED): \
                 mb_power_stats[0].getattr(ComputeMbPowerStats.TIME_COLLECTED)
                    }
                return power_stats
        return None
            ...
            for server in ls_server:
                ...
                if firmware_ver:
                    for ver in firmware_ver:
                       params = {
                           FirmwareStatus.DN: ver.getattr(FirmwareStatus.DN),
                           FirmwareStatus.OPER_STATE: \
                               ver.getattr(FirmwareStatus.OPER_STATE),
                           FirmwareStatus.PACKAGE_VERSION: \
                               ver.getattr(FirmwareStatus.PACKAGE_VERSION)
                           }
                ...
                if compute_unit:
                    if mo_id is ComputeBlade.ClassId():
                        for unit in compute_unit:
                            params = {
                                ComputeBlade.DN: \
                                    unit.getattr(ComputeBlade.DN),
                                ComputeBlade.CHASSIS_ID: \
                                    unit.getattr(ComputeBlade.CHASSIS_ID),
                                ComputeBlade.AVAILABLE_MEMORY: \
                                   unit.getattr(ComputeBlade.AVAILABLE_MEMORY),
                                ComputeBlade.NUM_OF_ADAPTORS: \
                                    unit.getattr(ComputeBlade.NUM_OF_ADAPTORS),
                                ComputeBlade.NUM_OF_CORES: \
                                    unit.getattr(ComputeBlade.NUM_OF_CORES),
                                ComputeBlade.NUM_OF_CORES_ENABLED: \
                               unit.getattr(ComputeBlade.NUM_OF_CORES_ENABLED),
                                ComputeBlade.NUM_OF_CPUS: \
                                    unit.getattr(ComputeBlade.NUM_OF_CPUS),
                                ComputeBlade.NUM_OF_ETH_HOST_IFS: \
                                unit.getattr(ComputeBlade.NUM_OF_ETH_HOST_IFS),
                                ComputeBlade.NUM_OF_FC_HOST_IFS: \
                                 unit.getattr(ComputeBlade.NUM_OF_FC_HOST_IFS),
                                ComputeBlade.NUM_OF_THREADS: \
                                    unit.getattr(ComputeBlade.NUM_OF_THREADS),
                                'ProcessorUnits': \
                        ...
                        for unit in compute_unit:
                            params = {
                                ComputeRackUnit.DN: \
                                    unit.getattr(ComputeRackUnit.DN),
                                ComputeRackUnit.AVAILABLE_MEMORY: \
                                unit.getattr(ComputeRackUnit.AVAILABLE_MEMORY),
                                ComputeRackUnit.NUM_OF_ADAPTORS: \
                                 unit.getattr(ComputeRackUnit.NUM_OF_ADAPTORS),
                                ComputeRackUnit.NUM_OF_CORES: \
                                    unit.getattr(ComputeRackUnit.NUM_OF_CORES),
                                ComputeRackUnit.NUM_OF_CORES_ENABLED: \
                            unit.getattr(ComputeRackUnit.NUM_OF_CORES_ENABLED),
                                ComputeRackUnit.NUM_OF_CPUS: \
                                    unit.getattr(ComputeRackUnit.NUM_OF_CPUS),
                                ComputeRackUnit.NUM_OF_ETH_HOST_IFS: \
                             unit.getattr(ComputeRackUnit.NUM_OF_ETH_HOST_IFS),
                                ComputeRackUnit.NUM_OF_FC_HOST_IFS: \
                              unit.getattr(ComputeRackUnit.NUM_OF_FC_HOST_IFS),
                                ComputeRackUnit.NUM_OF_THREADS: \
                                  unit.getattr(ComputeRackUnit.NUM_OF_THREADS),
                                'ProcessorUnits': \
                                    get_processor_units(handle, unit),
                                'MemoryArrays': \
                                    get_memory_inventory(handle, unit),
...
class VendorPassthru(base.VendorInterface):
    ...
    def _enroll_nodes(self, context, **kwargs):
        ...
        handle = UcsHandle()
        try:
            ret_val = handle.Login(ucs_node['hostname'],
                          ucs_node['username'],
                          ucs_node['password'])
            if ret_val is True :
