...
class TestAttachDetachVif(test_api_base.BaseApiTest):
    ...
    @mock.patch("pecan.request")
    def test_vif_type(self, mock_req):
        mock_req.version.minor = versions.MINOR_MAX_VERSION
        vif_id = uuidutils.generate_uuid()
        request_body = {
            'id': vif_id,
            'misc': 'Something extra'
        }
        vif = api_node.viftype.frombasetype(request_body)
        self.assertEqual(vif['id'], vif_id)
        self.assertEqual(vif['misc'], 'Something extra')

    @mock.patch("pecan.request")
    def test_vif_type_bad_id(self, mock_req):
        mock_req.version.minor = versions.MINOR_MAX_VERSION
        self.assertRaises(exception.InvalidUuidOrName,
                          api_node.viftype.frombasetype, {'id': 5678})
